...
function rsAdjustGrade(grade, rsResult) {
  ...
  function wireBX(h) {
    ...
    // Init _lastLiveST from latest history entry (or entryST) so recording without
    // re-clicking ST still carries the most-recently chosen direction.
    const latestHistST = h.bxHistory?.length ? h.bxHistory[h.bxHistory.length - 1].st : undefined;
    const initST = latestHistST !== undefined ? latestHistST : (h.bx?.entryST ?? null);
    if (initST === true || initST === false) {
      _lastLiveST = initST;
      const stVal = String(initST);
      $$("[data-drawer-st]", dr).forEach(b => b.classList.toggle("active", b.dataset.drawerSt === stVal));
    }
  ...
  let simTradeLogCollapsed = localStorage.getItem("trendo_sim_tradelog_collapsed") === "1";
  ...
  function applyCloudData(data) {
    ...
    if (data.cycleCheck && typeof data.cycleCheck === "object") {
      ...
      CYCLE_CHECK.history        = data.cycleCheck.history        || [];
    }
    ...
    try {
      ...
      if (cc) { try { const p = JSON.parse(cc);
        if (p && typeof p === "object") { CYCLE_CHECK.items = p.items || {}; CYCLE_CHECK.log = p.log || [];
          CYCLE_CHECK.confirmedPhase = p.confirmedPhase || null; CYCLE_CHECK.confirmedAt = p.confirmedAt || null;
          CYCLE_CHECK.history = p.history || []; } } catch (_) {} }
      ...
      return `
        <div class="pp-prog-wrap">
          <div class="pp-prog-track">
            <div class="pp-prog-fill${prog >= 1 ? " done" : ""}" style="width:${(prog*100).toFixed(1)}%"></div>
          </div>
  ...
  function wireNewPositionModal() {
    ...
    const _updateSizer = () => {
      ...
      // Resolve entry price: estimated for market, limitPrice for limit, direct for manual
      let entry;
    ...
    if (_sizerHint) {
      _sizerHint.addEventListener("click", e => {
    ...
    if (_sizerAtrIn) {
      _sizerAtrIn.addEventListener("input", () => {
        ...
        const refEntry = orderMode === "market"
          ? (parseFloat($("#form-est-entry")?.value) || parseFloat($("#form-entry")?.value))
          : orderMode === "limit"
            ? parseFloat($("#form-limit-price")?.value)
            : parseFloat($("#form-entry")?.value);
    ...
    const STRATS = [
      { icon: "📈", group: "看涨", sub: "Bullish", strats: [
        ...
        { name: "Bull Call Spread", zh: "牛市看涨价差",
          prin: "以卖出更高行权价Call来抵消买Call成本，在定义范围内博涨，IV高时卖出份权利金厚",
        ...
        { name: "Risk Reversal",    zh: "风险逆转",
          ...
          timing: "强烈看涨 · 愿意承担下行 · 两份权利金接近" },
      ]},
      { icon: "📉", group: "看跌 / 对冲", sub: "Bearish · Hedge", strats: [
        { name: "Bear Call Spread", zh: "熊市看涨价差",
          prin: "卖低行权价Call收权利金，买高行权价Call封顶风险；价格不超过卖出份行权价则全收",
        ...
        { name: "Collar",           zh: "领口策略",
          ...
          timing: "持仓大幅盈利需保护 · IV偏高（卖出份收益厚）" },
      ]},
      { icon: "↔️", group: "横盘 Neutral", sub: "Neutral · Income", strats: [
        { name: "CC",              zh: "备兑看涨",
  ...
  function renderOptions() {
    ...
    const positionsHTML = `
         ...
         ${open.length ? (() => {
           ...
           return (oCsp.length ? `<div class="opts-sub-label opts-sub-strat">CSP · ${oCsp.length}</div>${oCsp.map(_optOpenPosCard).join("")}` : "") +
  ...
  function simbBarRows(entries, labelOf, colorOf) {
    const maxAbs = Math.max(1, ...entries.map(e => Math.abs(e.pnl)));
    return entries.map(e => {
      const barPct = Math.round(Math.abs(e.pnl) / maxAbs * 100);
      const barColor = e.pnl >= 0 ? "var(--up)" : "var(--down)";
  ...
  // 唯一的差别是数据来源数组和读取哪个 pnl 字段，以及顶部 3 张概览卡片的文案。
    ...
    const html = `
      <div class="simb-history">
        <div class="simb-history-hd">
          ...
          <span class="simb-history-en">Past Months</span>
          <span class="simb-history-rule"></span>
        </div>
        ${blocks}
      </div>`;
  ...
  function renderMonthlyBacktest({ labelSel, sectionSel, mode, sourceArr, otherArr, peakStorageKey, closedNote, closedNoteInline, notional = 0, titleZh = "月度回测", titleEn = "Monthly Backtest", scopeToMonth = true }) {
    ...
    if (!section) return;
    const hasAny = sourceArr.length > 0 || otherArr.length > 0;
    if (!hasAny) {
    ...
    const countLabel = scopeToMonth ? (mode === "closed" ? "月内已平仓交易" : "月内开仓总数") : "已平仓交易数";
  ...
  function renderSimTradeLog() {
    ...
    if (labelEl) {
      ...
      if (toggleBtn) {
        ...
        toggleBtn.onclick = () => {
          simTradeLogCollapsed = !simTradeLogCollapsed;
          localStorage.setItem("trendo_sim_tradelog_collapsed", simTradeLogCollapsed ? "1" : "0");
          renderSimTradeLog();
        };
      }
    }
    if (!hasAny) { el.innerHTML = ""; return; }
    el.style.display = simTradeLogCollapsed ? "none" : "";
    if (simTradeLogCollapsed) return;
...
<div id="${chartId}-tip" class="ec-tooltip" style="display:none"></div>
  ...
  const MKT_ZONES = {
    vix: {
      ...
      zones: [
        ...
        { max: 50,  color: "var(--down)",  label: "恐慌 · 25%上限",   badge: "恐慌 25%" },
        { max: 9999,color: MKT_DEEP_DOWN,  label: "极恐 · 清仓观望",  badge: "极恐" },
      ]
    },
    vxn: {
      ...
      zones: [
        ...
        { max: 65,  color: "var(--down)",  label: "恐慌", badge: "恐慌" },
        { max: 9999,color: MKT_DEEP_DOWN,  label: "极恐", badge: "极恐" },
      ]
    },
    fg: {
  ...
  function mkPhaseHTML(ph, axes, scope = "YTD", pending = null, settledDate = null) {
    ...
    // No labels inside the bar. Across a full year most segments are too narrow to hold
    ...
    // Both bands explained on one line, since the bar itself now carries no text. Only
    // the phases actually present in this window are listed.
    const phaseKeys = [["tailwind", "做多"], ["neutral", "中性"], ["headwind", "做空"]]
  ...
  const CYCLE_TIER_DESC = {
    ...
    2: "T2 结构性：钱从哪来、产出值多少——硬结构，不易被情绪解释",
    3: "T3 行为性：会计手法与市场行为——真实，但都能用情绪解释掉",
  };
  const CYCLE_STATES = [
    { k: "clear", zh: "未出现" },
    { k: "watch", zh: "观察中" },
  ...
  const CYCLE_ITEMS = [
    ...
    { id: "capex_breadth", tier: 2, days: 90, zh: "AI 资本开支参与广度", short: "参与广度",
      what: "整轮周期能不能撑住，最终取决于愿意掏钱的人够不够多——如果只剩头部四五家自筹资金硬扛，边际买家已经先退场了。这条问的是「钱从哪来」里被其余判据忽略的一侧：`circular`/`debt_ratio` 测的是头部自己的融资结构，这条测的是头部之外还有没有新玩家愿意加码。",
      where: "二线云厂商/neocloud（CoreWeave、Nebius 等）的资本开支与融资渠道；主权财富基金对 AI 基建的配置比例；企业级 IT capex 调查",
      how: "头部四五家之外的参与者是在扩大，还是在退场？",
      warn: "「二线厂商靠借头部的钱买头部的芯片」这类关联融资算退场信号，不算广度扩大" },
  ];
  ...
  const CYCLE_BASELINE = {
    ...
    items: {
      ...
      capex_breadth: { state: "watch", note: "两条相反的证据并存：主权财富基金对基建的平均配置已从 2022 年约 5% 升到 2025 年约 9%，海湾主权基金是绝对金额上最积极的追加者，参与广度确实在扩大；但二线云厂商/neocloud（CoreWeave、Nebius 等）自己没有头部厂商那样的现金流护垫，扩张几乎全靠举债，且头部四五家（MSFT/GOOGL/AMZN/META/ORCL）2026 年资本开支合计约 6,600–6,900 亿美元，仍是其余所有参与者总和的数量级压制。广度在扩，但新增部分结构脆弱，记观察中。" },
    },
  };
  ...
  // 某条是否已过自己的复核周期（days=0 的事件型判据永不过期）
  function cycStaleOf(it) {
    const rec = CYCLE_CHECK.items[it.id];
    if (it.auto || !it.days || !rec || !rec.at) return false;
    return Math.floor((Date.now() - new Date(rec.at + "T00:00:00").getTime()) / 864e5) > it.days;
  }
  ...
  function cyclePhase() {
    ...
    // 临界：得分贴着某条分档线（±0.2 分）时如实标出——这种位置上，任何一条判据
  ...
  function cycItemHTML(it) {
    ...
    return `<div class="cyc-item cyc-st-${st}">
      <div class="cyc-hd">
        <span class="cyc-tier t${it.tier}" title="${CYCLE_TIER_DESC[it.tier]}">T${it.tier}</span>
  ...
  function cycHistHTML() {
    ...
    const dirTxt = Math.abs(delta) < 0.05 ? "基本持平"
  ...
  function renderCycleCard() {
    ...
    if (cycleSnapshot(cyclePhase())) dirty = true;
  ...
  async function fetchMarketData() {
    ...
    try {
      // The cycle card shows one trading year (252 sessions). EMA200 needs 200 sessions
      // before it produces its first value, so the request reaches back far enough to

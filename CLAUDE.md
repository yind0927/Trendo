...
**窗口 `ADVICE_SESSIONS = 200`（v800，不再跟阶段周期的 252 同窗口）**：这张卡最多 7 态，段数天生
比阶段周期（只有 3 态）多得多（压力测试一年能碎到 147 段），窗口越长手机端色带、图例、状态说明就
越密。200 个交易日（约 10 个月）是"看得出走向"与"手机端读得下去"之间的折中——两张卡回答的问题
不同，没必要强行同窗口才能比较。

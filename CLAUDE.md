...
| v508 | **修复持仓表头与内容错位**（真实仓+模拟仓通病）：根因是 `<thead>` 只为 `COLS` 生成表头 `<th>`，但每行 `<tbody>` 都多一个"操作"列 `<td>`（关闭/删除/撤回按钮），导致表头比数据行少一列；在 `table-layout:auto` 下浏览器按内容自动分配列宽，长公司名换行/徽章描边撑宽某列后头尾即视觉错位。修法（与 v7.9 Market 页三表同款）：表头补一个空 `<th class="th-actions">`占位对齐操作列；`table.holdings`/`table.sim-holdings` 改 `table-layout:fixed`；新增 `<colgroup>`（`#holdings-colgroup`/`#sim-holdings-colgroup`）由 `colgroupHTML()` 按 `COL_WEIGHT` 权重表动态生成各列百分比宽度（含操作列，随用户勾选显示的列自动重新分配，不会因隐藏止损/止盈等列而错位）；`th[data-col]`/`th.th-actions` 点击排序逻辑排除新占位表头，避免误触发排序。 |

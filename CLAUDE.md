...
### 建议走向（`mkAdviceHTML`，v799 起，v800 调整窗口与加说明）—— 综合建议的逐日回放
...
**状态说明列表（`.mkp-advice-defs`，v800 新增）**：图例只有颜色+天数+占比，不解释"防守"/"止盈"这些
标题本身是什么意思。新增的说明列表**只列这段窗口里实际出现过的状态**（不是把 7 种可能状态全列
一遍——没出现过的跟当前无关，列出来只增加要读的行数），每条给「名称 + state（为什么）+ detail
（怎么做）」三段，内容直接取自 `combineAxes` 返回的 `state`/`detail` 字段（回放时随 `day` 对象
一起存下来），不是另造的文案，跟横幅、色带 tooltip 用的是同一份措辞。手机端三段纵向堆叠（名称一行，
state/detail 各占一行），不像桌面端那样并排——横向挤在 390px 里会重叠。

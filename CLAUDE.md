...
window.REAL_OPTIONS      // 期权仓位 []（CSP/CC 卖方，手动记录模型）— Options 页唯一数据源
                         //   模拟期权（SIM_OPTIONS）已整体删除：全局变量、localStorage
                         //   键、云同步字段、服务端种子数据全部移除，不要再加回来
...
**日期一律按本地自然日解析**：`"YYYY-MM-DD"` 交给 `new Date()` 会按 UTC 解析，美洲负时区格式化时整体回退一天。`fmt.date()` 与 `parseLocalDate()` 都已逐字段解析，新写日期展示时别再直接 `new Date(isoDateString)`。
...
**列的页签可见性**：`COLS` 上的 `closedHide`（仅持仓中出现，如止损/止盈）与 `openHide`（仅已平仓出现，如平仓日期）由 `visTableCols(isClosed)` 一处处理，现持仓与模拟仓两张表共用；新增这类列时记得同步 `COL_WEIGHT`（colgroup 宽度）与手机端 `.closed-tab` 的 `min-width`。
...
// combineAxes(dir,risk,sent) → { id, headline, state, emoji, color, detail } 综合建议
//   headline 说「怎么做」（防守/止盈/兑现/布局/分批参与/保持持仓/正常配置，共 7 档——
//   旧文档这里漏过"兑现"，注意 trim 与 cooldown 是两个不同分支，别看错成一档）
//   state 说「为什么」（趋势逆风/极端过热/恐慌降温期/恐慌积累/情绪偏冷/情绪偏热/趋势顺风）
//   方向逆风=闸门（禁新仓）> 情绪过热=止盈倾斜 > 恐慌降温=兑现倾斜 > 加仓倾斜 > 正常进攻
```
...
- `combined.id`（defense/trim/cooldown/accumulate/scale/hold/normal）是**稳定标识**，专供周期分析卡色带分段。
  `headline` 是给人读的文案、改过多版，拿它当分段键会让文案一改历史就碎成两段——别这么干。
- **颜色（v803）**：`trim`(止盈) 与 `cooldown`(兑现) 此前共用 `var(--orange)`，横幅/图例/色带里读不出区别——
  两者触发条件、紧迫程度都不同（止盈=情绪过热主动减仓，兑现=VIX 从恐慌回落后了结，更从容）。`cooldown`
  改用调色板里另一个未被 `combineAxes` 占用的色相 `var(--blue)`（`getSentimentAxis` 里同名的情绪轴状态
...
**模块名与位置（v801）**：卡片标题「建议走向 · Advice History」改为「**周期分析 · Cycle Analysis**」；
渲染顺序从"综合建议横幅正下方"移到**阶段周期卡正上方**（`renderMarket` 里 `mkAdviceHTML(advice)` 紧接在
`mkPhaseHTML(...)` 之前），VIX/VXN 等指标行退回综合建议横幅下方——两张回放色带卡（周期分析 + 阶段周期）
现在挨在一起，方便对照读。**注意**：Market 页另有一张同名相近的「周期系统分析 · CYCLE SYSTEM」卡
（`renderCycleCard`，人工季度判据清单，v760 起），跟这张自动回放的「周期分析」是两个完全不同的模块，
名字接近纯属用户指定命名、并非本意合并，改这两张卡时留意别认错。

**窗口 `ADVICE_SESSIONS = 200`（v800 收窄，v803 起阶段周期的 `PHASE_SESSIONS` 也改回 200 保持同步）**：
v800 当时的理由是这张卡最多 7 态、段数天生比阶段周期（只有 3 态）多得多（压力测试一年能碎到 147 段），
所以先把这张卡的窗口单独收窄、不强求两卡同窗口。v801 把两张卡挪到相邻位置后，"挨在一起却看不同
时间段"反而更容易读错；v803 用户明确要求两卡同窗口，于是把 `PHASE_SESSIONS`（阶段周期，定义在
...
新增 `ADVICE_RULES` 常量（与 `combineAxes` 函数体分开维护，一份是代码判断、一份是给人看的文字版，
**改 `combineAxes` 任何一条分支时要记得同步这张表**，两处脱节过一次就会互相矛盾），按优先级顺序
...
采用该结果——方向逆风是闸门，一票否决；其余按情绪信号从上到下排列优先级"），避免有人把这份列表读成
互斥并列条件（实际是短路判断，`normal` 是"以上条件均不满足"的兜底，不是它自己有一条独立触发规则）。

**头部文案与顶部「综合建议」横幅强制一致（v802）**：`mkAdviceHTML(ah, live)` 新增第二参数
`live`（调用处传 `axes.combined`，跟顶部横幅 `mkAxesHTML` 用的是**同一个对象**）。此前卡片头部
（`.mkp-now`/`.mkp-axis-now`）直接展示 `ah.current`——那是**纯粹用已收盘的历史 K 线回放**出来的
最后一段，而顶部横幅用的是**盘中实时价**（`vooLive`）算出的 `combined`；FGI 更是两个不同来源
（横幅用 `/api/feargreed` 的 `score` 今天读数，回放用 `history` 里逐日的历史值，History 端点甚至
可能压根不含"今天"这一天）——收盘前两者不同步是常态，此前卡片会显示一个跟顶部横幅文案对不上的
状态，读者会当成"两处算出了不一样的结论"。现在头部统一显示 `live`（有 `live` 就必然跟横幅逐字一致，
没有才退回 `ah.current` 兜底）；`live.id !== ah.current.id` 时追加 `.mkp-pending` 提示条（与阶段周期
卡的 `pending` 机制同一个模式：「按当前三轴规则已进入 X —— 下方色带只画已收盘的交易日，这一笔要等
收盘才计入」），如实说明色带暂时没画上这一天、不是算错了。**状态说明清单联动**：若 `live` 的状态
还没进过历史色带的统计（`defsSource` 用 `pendingMismatch` 判断，把 `live` 的定义插到清单最前面），
保证展开说明时头部正在显示的那句话一定能在清单里找到解释，不会出现"头部写了一个词、清单里却没有
这个词"的情况。Playwright 回归专门构造了 FGI 历史值(50/中性)与"今天"读数(90/过热)不同、且方向轴
全程顺风（不会被防守闸门短路）的数据集，验证头部与横幅逐字一致、pending 提示正确出现、状态说明
清单能对上头部——用的是共享数据集会被防守闸门短路掉的教训（第一版测试数据集末尾恰好跌破 EMA200，
combineAxes 的防守分支在检查情绪之前就短路返回，fg 怎么改都测不出差异，因此另起了一份独立数据集）。
...
精确地每天在 59/61 跳这种病态输入，200 天窗口）：147 段左右的量级、切换列表仍只列个位数到十几条，
图例的占比在色带碎掉时接管了"主要待在哪个状态"这个问题。

**与下一模块的间距**：卡片外层 `.mkt-card` 单独使用（不在 `.mkt-row` 的 grid 里）时本身不带
`margin-bottom`，此前紧跟着的 VIX/VXN 那一排是零间距贴着的——v800 补了 `.mkt-advice { margin-bottom:
20px }`，跟阶段周期卡（`.mkt-phase` 同款规则）保持一致的模块间距。
...
## 模型选股（Sim 页子tab，v797 起有文档）

回答的问题是「按某个来源（AI/筛选器/别人推荐）每周挑一批股票，长期跟踪它跑不跑得赢 VOO」。
`window.MODEL_PICKS` 是一个**按周分批**的数组，`unshift()` 插入所以新批次在数组最前面：

```js
{
  id,       // "YYYY-Www" 周ID，mpIsoWeek() 生成，同一周重复录入会追加进同一批而不是新建
  weekOf,   // 录入当天的美股交易日 "YYYY-MM-DD"
  pickedAt, // ISO 时间戳
  source,   // 来源备注（可选）
  picks: [{ sym, name, entryPrice, entryDate, checkpoints: {}, lastPx, lastDay }],
  bench: { sym: "VOO", entryPrice, entryDate, checkpoints: {} }, // 每批自己配一份 VOO 基准
}
```

**入场价**：不是录入当天的价格，而是**入场日开盘价**（`mpRefresh()` 异步从 `/api/history` 的
`openResults` 取，到了才回填，回填前 `entryPrice` 为 null、显示"等待开盘"）。**检查点**
`MP_CHECKPOINTS = [d5,d10,d20,d30,d40,d60]`，按**入场日之后第 N 个交易日**（不是自然日）算收益率，
`d10`÷5=2 周是 `MP_PRIMARY`（主口径）。一旦写入某个检查点就**永久冻结**，不会因为后续价格变化重算。

### 顶部六张卡片（`.mp-stat-grid`）——六个持有期，不是六批股票

**这是最容易看错的地方**：六张卡对应 `MP_CHECKPOINTS` 的六个**持有期**（1/2/4/6/8/12 周），
**每一张都汇总全部已录入的批次**，不是「第一批」「第二批」……六批的意思。第 N 张卡回答的是
「所有已经**活过 N 周**的批次，平均比 VOO 多赚多少」——`mpStats()` 遍历 `MODEL_PICKS`，对每个
持有期分别把「已经走到这个检查点」的批次拎出来算平均超额收益（`mp-stat-val`）、批次胜率、以及
拆到个股层面的胜率/中位数/均值。**没走到该持有期的批次会被跳过、不计入分母**（不是当 0
处理），所以每张卡下方的"已走满 N 批"样本量本身就在变化——新录入的批次要过 1 周才会进第一张卡
的样本，过 12 周才会进最后一张。卡片自带一行说明（`.mp-stats-hint`）就是讲这件事，但读起来容易
被当成套话跳过。样本不足 `MP_MIN_N`(20) 批时顶部会有黄色警示，说明"同一周的选股一起涨跌，
它们不是彼此独立的证据"——20 批以内这些数字统计意义都很弱。

### 每批（周）的折叠卡头部——纯数字 `N 周`（v798）

批次列表（下方按周展开的 `<details class="mp-cohort">`）每一条的头部标签由 `mpHeadlineKey(c)`
决定该批**现在该用哪个持有期汇总展示**，标签只显示数字，不带任何后缀或 title：

```
一个检查点都没到（刚录入）→ "0 周"
只走到比主口径短的持有期  → "N 周"（N 是已完成的最长持有期）
已走到主口径(2周)及以上   → "2 周"
```

v797 曾给这三种状态分别加"已完成"/"不足 1 周"等说明文字（当时是为了跟 v797 之前"零数据
错误复用主口径 2 周文案"的 bug 划清界限），用户反馈"给出对应的几周就行，其他的不用写"，
v798 改为纯数字——零数据天然显示"0 周"，跟"1 周""2 周"本身就不会撞在一起，不需要额外文字
再区分一次。

**这不是"第一批/第二批/第三批"的批次序号**，纯粹是"这一批到今天为止能报出的最长持有期"，
同一批会随着时间推移从"0 周"→"1 周"→"2 周"（主口径，一旦命中就锁定显示主口径，不再随更
长周期到账而改变字面）逐步变化，跟它在列表里排第几、是第几批完全无关——刚录入的最新一批
显示"0 周"、上上周的老批次显示"2 周"是正常现象，不是乱序或 bug。

`hzGrid(c)`（展开后的六格明细）不受这个头部标签影响，六个持有期各自独立展示、`pending` 未到
的一律显示 `—`，那里没有这个歧义。
...
| v770 | **档位徽章改回 T1/T2/T3、删除时序标签 + 穷举复验新权重 + 过期项拉低完整度 + 得分走向**。**①徽章与标签**（按要求）——`.cyc-tier` 从「决定性 3 / 结构性 2 / 行为性 1」改回简洁的 `T1/T2/T3`（含义移到 `title` 与规则行，`cursor:help`），`when` 字段与 `.cyc-when` 时序 chip（领先/同步/终点）**整条删除**。v769 定下的**权重分配不变**（T1 决定性 3 分 ×2 · T2 结构性 2 分 ×5 · T3 行为性 1 分 ×5，满分 21），只是不再把「决定性」三个字印在每张卡上。**②穷举复验（改权重就得重验，v761 立的规矩）**——`enum769.js` 跑完 3¹² = 531,441 种状态组合：新权重下阶段分布为 **第3 20.6% · 第4 22.5% · 第5 22.9% · 第6 33.3%**，与 v768 旧权重 2/8/2 的 18.7/24.6/23.0/33.3 几乎一致，**10 分制分档线 6.0/3.5/1.5 无需调整**（第 6 阶段那 33.3% 恒等于「capex_guide 亮」占全组合的 1/3，按设计如此）。顺带量出得分分位 p25=3.49 / p50=4.44 / p75=5.40——**p25 几乎正压在第 4 阶段的 3.5 线上**，这解释了为什么当前读数 3.65 会贴着分档线，是量表的结构特征而非偶然。**③过期项在完整度里按半权**——这是回答「可信度如何定义」时自己点出的那道缝：此前一条半年前填的和今天刚核实的完全同权，「完整度」这个数对不起它的名字。新增 `cycStaleOf(it)`（按每条自己的 `days` 周期；`days:0` 的事件型判据永不过期），过期项 `wDone += w/2`。**刻意只打折完整度、不打折得分**——`maxV`（得分的分母）保持满权：过期不代表那条的事实变了，只代表没人再去看过，不该连带改掉打分口径；这条取舍直接写在 `.cyc-conf-def` 里。同时新增顶部 `.cyc-stale-sum`「⏱ N 条已过复核周期」汇总（此前只有每条自己的「已 N 天未复核」，收起状态下完全看不见），收起态摘要行也加 `.cyc-sum-stale`「N 条待复核」小标签。**④得分走向 `.cyc-hist`（本版最实质的补充）**——v760 就写过「趋势比快照有意义」并加了逐条文字变更记录，但**从来没有一条可比的得分序列**，等于看不出方向；而这正是周期模块唯一真正有用的东西。新增 `CYCLE_CHECK.history[]`（四处存储同步 + `data.js` 默认值）与 `cycleSnapshot(p)`：每天最多一条，同日内得分变化 ≥0.05 或阶段/完整度变了才覆盖，上限 60 条——`renderCycleCard()` 会被 30 秒一次的价格轮询调用，这两道闸是防止记录被刷爆的关键。`cycHistHTML()` 渲染折叠块：顶部一行走向摘要（`2026-06-30 → 2026-09-17：2.1 → 3.6 分 · 上行 +1.55 分（更接近周期尾声）`，**上行着风险色**因为得分升高意味着更接近尾声），下方逐条日期+进度条+得分+阶段；不足两条时如实说明「还只有 N 个读数」而不是画一条假趋势线。Playwright 回归（新增 `v770.js` 24 项）：徽章 T1/T2/T3 且仍是 2/5/5、时序 chip 归零、档位含义留在 title；构造两条过期（信用利差 90 天前 / capex 200 天前）实测完整度 18.5/21 = 88%、得分分母未打折仍 /21、顶部与收起态双处提示、两条各自标记；首次渲染写入 1 条快照且含得分与阶段、注入两条历史后走向摘要的起止与幅度（+1.55）均正确、同日不重复写跨日追加；手机端无溢出。`cycle2.js` 43 / `cycle3.js` 21 / `cycle5.js` 60 / `v768ui.js` 12 / `v769.js` 20 全过（v769 里测徽章样式与时序标签的 5 项迁至 v770.js）。 |
| v771 | **新增「距下一阶段 · WHAT WOULD CHANGE IT」——把这张表从描述变成监视清单**。此前卡片只告诉你现在在第几阶段，等于给了结论却没给下一步；现在 `cycNextSteps(p)`/`cycNextHTML(p)` 算出**最少需要哪几条改变状态才会跨线**，两个方向都算。**①上行**：到下一档还差多少分 + 最少需要点亮哪几条（贪心按单条增益从大到小取，`clear→lit` 得满权、`watch→lit` 得 2w/3）。**②下行**：距下面那条线只差多少 + 最少需要哪几条回落（`lit→watch` 掉 2w/3、`watch→clear` 掉 w/3）——当前读数 3.65 距 3.5 线只有 0.15 分，**下行其实才是更相关的方向**，此前卡片完全没表达这件事。**③两条硬规则单列**（`.cyc-next-hard`）：capex 指引下调 → 直接判第 6 阶段、不看分数也不受完整度门控；半导体订单掉头 → 阶段下限直接抬到第 5 阶段。它们完全绕开分数，攒再多 T2/T3 也不如这两条里的任何一条——已点亮的不再重复提示。**④自动项带标记**：宽度背离进入清单时标「自动项」，提醒它不是手动能改的。**⑤首测抓到一个真 bug**：下行那行原本写「要掉回第 4 阶段（3.5 分线）」——但 3.5 是**当前所处**那一档的门槛，跌破它落到的是**再下一档**，正确文案应是「掉回第 3 阶段」。`CYCLE_RATIO_CUT` 里 `dnCut[1]` 是当前档的编号而非跌落后的编号，直接用就会把方向说反；修为取 `asc` 里比 `dnCut` 更低的那一条（没有则落到第 2 阶段「尚无足够信号」），并把文案改为明确的「越过 X 分线 / 跌破 X 分线」。**⑥明确边界**：底部注明算的是**最少条数**（先挑单条影响最大的）、不是唯一路径；**「未填」的判据不参与这个计算**——它们一改会同时动分子和分母，不是单纯的加减分，混进来会让「还差多少分」这个数失真。Playwright 回归（新增 `v771.js` 25 项）：默认基线上行 2.35 分/最少 2 条（capex+半导体）、下行 0.15 分/最少 1 条（信用利差→观察中）且目标档位与「跌破 3.5 分线」文案正确；构造只剩三条轻权重判据的场景逼出宽度背离，验证「自动项」标记；高分场景（15/21 = 7.1 分、第 5 阶段）不再显示上行行（往上只能靠硬规则）且下行正确写「跌破 6.0 分线 → 第 4 阶段」；capex 点亮后（第 6 阶段）硬规则不再重复提示它、未点亮的半导体订单仍提示；手机端无溢出。`cycle2.js` 43 / `cycle3.js` 21 / `cycle5.js` 60 / `v768ui.js` 12 / `v769.js` 20 / `v770.js` 24 全过。 |
| v772 | **算力单价 → 10年期实际利率（计入评分）+ 数字项阈值泛化**（用户：「还是使用利率作为评分并且删掉算力价格，认为环境利率还是有因素」）。**①先纠正我自己之前的一个混淆**：v763/v766 反对把利率计入评分，理由是「FRED 给的是广谱 IG/HY 利差，会与 `credit_spread` 重复计分」——那个反对针对的是**信用利差**、至今仍成立；但**无风险实际利率**是另一回事：`credit_spread` 测的是**信用溢价**（市场要为「借给 AI 这个行业」额外收多少，如 Oracle CDS 75→218bp），实际利率测的是**资金本身的价格**，即使信用零风险也要付，它抬高的是**所有**长久期 capex 的门槛收益率。两者不重叠，此前我把它们混为一谈了。**②新增 T2 数字项「10年期实际利率（融资成本门槛）」**：`cuts:[1.5, 2.5]`、`step:0.05`、单位 %，阈值 <1.5% 未出现 · 1.5–2.5% 观察中 · >2.5% 已触发（参照系：这轮 AI capex 的投资决策大多在实际利率 1.5–2% 时做出，2.5% 以上长回本期项目的净现值明显承压）。基线 **2.15% → 观察中**。`warn` 明确写「别填名义利率（^TNX）」——名义利率含通胀预期、跟资金真实成本不是一回事。**数据源用手填而非自动拉取**：FRED 从 Vercel 不可达已由 v763–v766 连续三版验证，而这本来就是季度级人工清单，手填与 `debt_ratio` 同一套交互。**③删除「算力单价转跌」并说明代价**：它是 v768 加的、整张表**唯一一条测产出物价格**的判据，删掉等于重开那个缺口；缓解因素是 `ai_roi`（投入产出比）仍盖着产出侧的一半，缺口不是全空。**④数字项阈值泛化**（本版的结构性改动）：`cycNumState(v)` 原本把债务融资占比的 25/40 写死在函数里，现在有两条量纲完全不同的数字判据，必须由条目自带 `cuts:[watchMin, litMin]`；五个调用点全部补传条目，输入框的 `step`/`min`/`max`/单位/阈值刻度也一并改为条目驱动（此前 `<25 / 25–40 / >40` 是硬编码在 HTML 里的）。**⑤满分与判定均未变**：换出换入同为 T2、同为观察中，满分仍 21、得分仍 **3.6/10 → 第 4 阶段·金融化**、仍在临界（距 3.5 线 0.15 分）。**但这一条现在是实打实的承重条目**——实测把利率改到 1.2% 得分掉到 3.3 → **压回第 3 阶段**，改到 2.9% 升到 4.2；也就是说利率环境单独一条就能在当前位置翻动判定，这正是用户希望它被计入的理由。Playwright 回归（新增 `v772.js` 27 项）：12 条不变、算力单价确认删除、利率条 T2/2.15%/观察中/刻度 `<1.5 1.5–2.5 >2.5`/步长 0.05、核实说明写明与信用利差不重叠、债务融资占比的 25/40 与步长 1 不受泛化影响、满分 21 与得分 3.6 不变、三档阈值实测（1.2%→未出现→3.3→第3阶段 / 2.9%→已触发→4.2）且状态变化写进变更记录、手机端无溢出。`cycle2.js` 43 / `cycle3.js` 21 / `cycle5.js` 60 / `v768ui.js` 12 / `v769.js` 20 / `v770.js` 24 / `v771.js` 25 全过（v770/v771 的种子把数字项只给 `state` 不给 `value` 会被判「未填」，已各补一个数值——这与 v770 自测时踩到的是同一个陷阱）。 |
| v773 | **利率判据改为美联储政策利率 + 全模块文案统一到 10 分制**（用户：「这里说的利率是美联储加息降息的利率，并且现在文案和计算是不是相同的按照 10 分来的，因为有些地方还在使用百分制」）。**①利率条重做**——v772 我实现的是 10 年期 TIPS 实际利率，用户要的是**联邦基金利率目标区间**。两者都在测「钱本身多贵」，但政策利率更贴这轮 capex 的实际融资路径：超大厂发债、SPV 项目融资、私募信贷的定价全都锚在它上面，而且它是**可被人为调整**的那一个——加息降息是决策，实际利率是市场的结果。`real_rate` → `fed_rate`，`cuts` 1.5/2.5 → **3/4.5**（3% 附近是 FOMC 点阵图的长期中性估计），`step` 0.05 → **0.25**（美联储调息的最小单位），填**目标区间上限**（3.75–4.00% 就填 4.00）。基线 **4.00 → 观察中**——与 v772 同为 T2 观察中，故满分仍 21、得分仍 **3.6/10 → 第 4 阶段·金融化**、仍在临界（距 3.5 线 0.15 分）。**方向不进评分、但写进备注**：同样是 4%，处在加息通道里比降息通道里对 capex 的压制更强，而「方向」没法用一个阈值表达，硬塞进打分会把两件事混成一个数——改为在 `warn` 与基线备注里明确点出当前是**加息**通道（本轮以抑制通胀为首要目标、就业端未恶化），读卡片时一并看。**这一条仍是承重条目**：实测降到 2.5% 得分掉到 3.3 → 压回第 3 阶段，升到 5.0% 得分 4.2。**②全模块百分制残留审计**——v769 把总分改成 10 分制时，只改了计算与主要渲染，`CYCLE_PHASE_DEFS` 的四条「清单长相」文案（v768 写的）还停留在旧口径：「比例 <15% / 15–35% / 35–60% / ≥60%」，与卡片上「得分 ≥6.0 / ≥3.5 / ≥1.5」的规则行对不上，读者拿百分比去对分数必然对不上号。改为 **得分 <1.5 / 1.5–3.5 / 3.5–6.0 / ≥6.0**；两处代码注释（`只看比例` / `比例贴着某条分档线（±2pp）`）同步改为得分与 ±0.2 分。**明确保留百分比的地方**（不是遗漏，是量纲本来就是百分比）：**证据完整度**及其门控线 `≥70% / 60–69% / <60%`（它算的确实是「填了多少」的比例）、两个数字判据自身的单位（债务融资占比 %、政策利率 %）、宽度背离的「跌超 3%/1%」比值变化、以及基线备注里引用的真实世界百分比。Playwright 回归（新增 `v773.js` 41 项）：条目名/T2/刻度 `<3 3–4.5 >4.5`/步长 0.25/基线 4.00→观察中、说明写明填上限与方向另看备注、满分 21 与得分 3.6 不变、四条阶段定义均为 10 分制且全卡片不再出现「比例 N%」式文案、2.5%→第 3 阶段与 5.0%→4.2 两档实测、手机端无溢出；`cycle2.js` 43 / `cycle3.js` 21 / `cycle5.js` 60 / `v768ui.js` 12 / `v769.js` 20 / `v770.js` 24 / `v771.js` 25 全过（四个套件的种子把 `real_rate` 改名并按新阈值调整了数值）。 |
| v774 | **修复模拟仓「删除后复活」+ 开盘判定改按美东挂钟**（用户报：删除仓位刷新后还在 / 休市挂的单开盘用 PWA 打开没成交）。**①删除复活的真正根因**——`simDeletePosition` 与挂单取消只是把条目从数组里 splice 掉，**没有留下任何「它被删过」的痕迹**；而 `syncOnStartup()` 在「本地更新」分支有一段**打捞逻辑**，会把「云端有、本地没有」的 `simHoldings`/`simPending`/`simClosePending` 补回本地（设计目的是救另一台设备刚建的单，不让盲推覆盖掉）。对打捞逻辑来说，「刚被删掉」和「另一台设备新建的」长得**一模一样**，于是每次同步都把刚删的东西捞回来，**并且接着 `saveLocalOnly()`+`syncPush()` 写回云端固化**——所以删了也回来、再删还回来。Playwright 先复现：删除 BBB → 触发一次 `visibilitychange` → `SIM_HOLDINGS` 从 `[AAA]` 变回 `[AAA,BBB]`；取消挂单 DDD 同样复活。**修法：删除墓碑** `SIM_TOMBSTONES`（`data.js` 新增全局 + `trendo_v4_sim_tombstones` + 四处存储同步 + 云同步字段 `simTombstones`）——删除/取消时记下 key（持仓用 `tradeIdOf`、挂单用 `order.id`），打捞与**整块覆盖两条路径**都按墓碑跳过；墓碑**双向合并**（吸收云端的，也推自己的），所以在任一台设备上删除都是终态；30 天过期 + 上限 200，过期后不再拦截以免挡住恰好复用同一 key 的新数据。**②自测抓到我自己写的一个数据丢失 bug**：最初我「顺手」给**成交**的挂单也立了墓碑（想着更严密），结果第 5 项测试报错——当云端 `savedAt` 更新触发整块覆盖、把本地刚成交的仓位抹掉时，原本挂单会被恢复、下一个 tick 重新成交而自愈；立了墓碑挂单就永远回不来，**仓位真的没了**。已撤销，并把这条取舍写进代码注释（成交本身已有证据——仓位就在 `SIM_HOLDINGS` 里，打捞靠 `openSyms` 就能跳过）。**墓碑只给用户主动删除/取消用**。**③开盘判定 `isUSMarketOpen()` 固定 UTC 窗口的错误**——旧版写死 `13:30–21:00 UTC` 并注释「EDT+EST 都覆盖」，覆盖是覆盖了但**太宽**：夏令时下 21:00 UTC = 美东 **17:00**，于是 16:00–17:00 的**盘后一小时被当成开盘，市价单会按盘后价成交**；冬令时下 13:30 UTC = 美东 **8:30**，盘前一小时同理。新增 `etNow()` 用 `Intl.DateTimeFormat` 取真实的美东星期/时分（`hour` 在午夜会给 "24"，已归一到 0），判定改为美东挂钟 **9:30–16:00**，夏令时切换自动跟随、不需要维护偏移表；`Intl` 不可用时退回旧的 UTC 近似而非整个失效。`api/order-check.js` 的同款门控一并修正。**④关于「挂单没成交」**：客户端成交链路本身经 Playwright 验证是好的（假时钟置于周一美东 10:00，20 个市价单全部成交、队列清空；反复切前台不回滚）。真正的缺口有两个——(a) `api/order-check.js` 这个后台成交 worker **从来没有被 `vercel.json` 里的任何 cron 引用过**（Hobby 计划不支持每分钟 cron），所以不打开页面就不会成交，这一点 v774 未改，需要外部定时器（cron-job.org 每分钟 GET `/api/order-check`）或 Pro 计划；(b) 盘后/盘前误判为开盘，已由 ③ 修好。Playwright 回归（新增 `v774.js` 29 项）：删除/取消在云端较旧与较新两条路径下都不复活、墓碑写入与推送、云端墓碑被吸收、5 单正常成交且成交**不**立墓碑、云端整块覆盖后能自愈回 5 持仓/0 挂单、开盘判定五档实测（夏令时盘中成交/夏令时 16:30 盘后不成交且徽章显示休市/冬令时 8:35 盘前不成交/冬令时盘中成交/感恩节不成交）、30 天前的墓碑被清掉、手机端无溢出。`cycle2.js` 43 / `cycle3.js` 21 / `cycle5.js` 60 / `v768ui.js` 12 / `v769.js` 20 / `v770.js` 24 / `v771.js` 25 / `v773.js` 43 全过（`v772.js` 在 v773 把 `real_rate` 改名为 `fed_rate` 时就已失效，本版删除——它测的内容已被 `v773.js` 完全覆盖）。 |
| v775 | **修复「休市刷新后挂单从队列消失，但那笔成交根本不该发生」**（用户补充的关键线索：PWA 打开并刷新，休市的挂单列表就没有了，以为成交了、其实没有）。v774 验证过的是**开盘时段的实时成交**，而这个问题出在另一条路径——`catchUpPendingOrders()`（v7xx 加的「补回放」：把 app 没开着时错过的整个交易日，用日线 open/low/high 补成交）。**根因：它只比较日期，完全不看下单的时刻。** 一笔周五 18:00（收盘后）下的市价单，`entryDate` 是周五，而周五那根日线已经收完、满足 `d < today`，于是被回放成**按周五 09:30 的开盘价成交**——用的是下单前好几个小时就已经定死的价格，`entry` 日期也写成周五。用户看到的就是：刷新一下，挂单从队列里消失了。**修法**：新增 `gateOf(order)`/`eligible(order, 日期)`，用订单自带的 `createdAt`（带时刻的 ISO）经 `etNow()` 换算成美东日期+分钟，一个交易日 D 只有在**订单已经存在、且 D 还没开盘**时才可回放——`下单日 < D`，或 `下单日 === D 且下单时刻 < 美东 9:30`（盘前下的单仍可吃当天开盘价）；开仓单与平仓单两处 `filter(s => s.d >= od)` 全部换成 `filter(s => eligible(order, s.d))`。**老订单没有 `createdAt` 时保守处理**：当天一律不回放，只认之后的交易日——宁可晚一天成交，也不按一个下单前就已确定的价格记一笔。**没有回避的代价**：这类单现在要等到下一个交易日真正走完才会补上（周五晚下的单，周一收盘后才回填为周一开盘价），在此之前它就该留在队列里——这是正确行为，队列里「休市 · 开盘后成交」的提示本来就是这个意思。Playwright 先复现再修（`repro3.js` 四个休市场景，修复前周五/周三收盘后下的单在刷新时被回填成当天开盘价、队列清空）；新增 `v775.js` 17 项：收盘后下单不回填当天且留在队列、下一交易日走完后按**周一** $70 而非周五 $60 成交且 `entry` 记为周一、盘前 08:00 下单仍可吃当天 $40 开盘价、无 `createdAt` 的老订单保守不回填、限价单三档（收盘后不吃当天 low / 未触及继续挂着 / 触及后按 min(开盘,限价) 成交）、平仓挂单同样门控且下一交易日按 $70 平仓、开盘时段实时成交仍按实时价 $66 成交不受影响。`v774.js` 29 / `cycle2.js` 43 / `cycle3.js` 21 / `cycle5.js` 60 / `v768ui.js` 12 / `v769.js` 20 / `v770.js` 24 / `v771.js` 25 / `v773.js` 43 全过。 |
| v776 | **盈利保护加「回撤45%」刻度 + 出场判定徽章对齐效率 chip + Analytics 顶部两张卡片建立层级**（三条用户需求）。**①盈利保护进度条新增第二道刻度**——原来轴上只有一条实心的保护价线（`入场价 + 60% MFE`，即回撤 40% 就该走）。新增 `.pp-axis-dd-line` 细虚线标在 **55%**（保留 55% 峰值盈利＝**回撤 45%**）+ 下方一行 `.pp-axis-dd-note`「回撤45% $X」，跌破时刻度与文案同时着 `--down` 并追加「已跌破」。**做成虚线+灰标签、跟实心的保护价线分主次**：保护价那条是触发线，这条只是「已经跌过头多少」的参考，不该抢注意力。实测入场 100 / MFE 40 → 刻度 55%、保护价线 60%、标注 $122.00。**②出场质量「出场后」那一行**：(a) 判定徽章（错误/正确/持平）从 v755 的**描边方角 + 内部竖线分隔**改为与行首「出场效率」chip（`.eq-eff-chip`）**完全同一套标签语言**——pill 圆角 10px、`padding:2px 8px`、10px/600、mono、填充底色（`--down-dim`/`--up-dim`/`--bg-3`）、无描边、无内部竖线。两者都是这一行的结论标签，此前一个描边方角一个填充圆角，并排看像两套控件；实测六项样式属性（圆角/内边距/字号/字重/字体族/描边宽度）与效率 chip 逐项一致。(b) **删除 `.eq-after-cmp`**「出场均价 $X → 第N日 $Y」——三个 cell 的百分比已经表达了同一件事，绝对价位在这一行里没有被用来做任何判断，只是让行更长（v757 加它是为了填右侧空白，v759 之后那个理由已不成立）。`eq-after-cmp` 的全部 CSS（含手机端 `display:none`）一并删除。**③Analytics 顶部两张卡片的信息层级**——问题与 v586 给月度回测修的是同一类：一张卡里 10 个 tile 全是同一字号同一样式，没有任何东西告诉你先看哪个。**关键发现：`rheroStat()` 函数与 `.rhero-row/.rhero-stat/.rhero-value` 全套 CSS 早就写好了（注释里连设计意图都写明了「竖线分隔的 KPI 条统一展示头条数字」），但从来没有被接进 HTML** —— 本版把它接上。卡片1「复盘概览」：头条 4 项（已实现盈亏 / 资金加权收益率 / 总体胜率 / 历史回撤 —— 赚没赚·回报率·准不准·最大回撤），下方 `.simb-subtitle`「明细 · Breakdown」+ 6 格 `cols-3`（总投入金额/资金利用率/已平仓笔数/盈利/亏损/持平）。卡片2「收益与盈亏」：头条 4 项（盈亏因子 / 平均收益率 / 中位数收益率 / 平均持仓 —— 每笔交易的质量），下方「盈亏构成 · Composition」4 格 + 「最佳 / 最差」。**头条数字不在下方明细里重复出现**。同时把卡片里并存的两套小节标题写法（`.simb-overview-row-label` 纯文字 / `.rrow-group`+`.simb-title`）统一成全站的 `.simb-subtitle`（teal 刻度线 + 中英双语 + 可选说明），与月度回测一致。**自测抓到一个「断言通过但设计没达成」**：首版接上后头条 19px、明细 tile 18px，`hero > tile` 的断言过了，但 1px 差距等于没有层级；`.rhero-value` 改为 **26px**（手机 21px）、`.rhero-row` 补 `border-bottom` 成为独立的一条带，断言同步改为要求差距 ≥6px。Playwright 回归（新增 `v776.js` 36 项）：刻度位置/价格/虚线/跌破着红各一组、判定徽章六项样式与效率 chip 逐项比对 + 确认页面不再出现「出场均价」、两张卡片头条项目与字号差、旧两套小节标题清零、头条不重复、手机端 390px 头条 2 列且无溢出。`v775.js` 17 / `v774.js` 29 / `cycle2.js` 43 / `cycle3.js` 21 / `cycle5.js` 60 / `v768ui.js` 12 / `v769.js` 20 / `v770.js` 24 / `v771.js` 25 / `v773.js` 43 全过；`eqafter.js` 的 3 条断言测的是已删除的 `.eq-after-cmp`（绝对价格与手算一致 / 与徽章间距 / 行尾空白收窄），改为「确认它确实不在了」后 20 项全过——该套件其余项测的是判定逻辑本身（错误/正确/持平三档与 ±3% 阈值），仍然有效，不删。 |
| v777 | **修正 v776 那条刻度：45% 是「捕获峰值的 45%」，不是「回吐 45%」——与出场质量同一套算法，并把设计做实**（用户：「应该是到峰值的 45% 的地方，也就是说和出场质量分析那里是相同的计算，并且现在设计不太强调」）。**①算法改对**——v776 我理解成「回吐 45% 峰值盈利」（保留 55%），刻度落在 55%。用户要的是**捕获峰值的 45%**，也就是 `exitQualityHTML` 里的 `efficiency = min(实际, 峰值) ÷ 峰值`。刻度改为 **45%**、价格 `cost + 0.45×MFE`。**这条轴本来就是出场效率**：0% = 入场成本、100% = 历史峰值，填充到现价的位置就是 `实际盈亏 ÷ 峰值盈亏`——跟出场质量那个 efficiency 是同一个算式，此前只是没把这层关系说出来。**②查出 45% 不是随便挑的数**：`exitQualityHTML` 的分档是 `efficiency > 70 ? high : efficiency >= 45 ? mid : low`——**45% 正是「低效出场」的分界线**。所以这条刻度的含义很明确：现价跌到它左边，这笔一旦出场就会被出场质量评为 low。文案据此改为「低效线 45% · $X」。**③设计做实**（v776 的细虚线 + 灰字太弱，读不出来那是条界线）——三件一起做：(a) `.pp-axis-lowzone` 把 0–45% **整段染成低效区**（`--down` 12%，跌入后加深到 26%）；(b) `.pp-axis-eff-line` 实心 2px 竖线（跌入后转纯 `--down`），轨道高度 6→8px 让染色读得出来；(c) 轴下新增 `.pp-axis-eff-row`：**当前效率 chip**「现在出场 75% 效率」（`.pp-eff-chip`，与 `.eq-eff-chip` **完全同一套样式与同一套分档**，实测 radius/padding/字号/字重/字体族/底色六项逐项相同）+ 低效线价格标签，跌入时整行转红并追加「已跌入」。**新增的「当前效率」本身是增量信息**——此前这张卡片从没说过「现在出场能拿到峰值的百分之几」，而这正是盈利保护要回答的问题。Playwright 回归（`v776.js` 扩到 46 项）：低效线与染色区实测均落在轨道 45%（按 `getBoundingClientRect` 反算）、保护价线仍 60%、价格 $118.00 = 100+0.45×40、当前效率 (130−100)/40 = 75% 归 high 档、跌到 $115 时 37.5%→38%（与出场质量同用 `Math.round`）归 low 档且三处同时强调、chip 六项样式与 `.eq-eff-chip` 逐项一致。**自测抓到两个我自己的测试 bug**（非产品）：`'pp-axis-lowzone'.includes('on')` 因为 `lowz-on-e` 含子串恒为真，改用 `classList.contains`；以及把 37.5% 的期望写成 37（`Math.round` 得 38）。`eqafter.js` 20 / `v775.js` 17 / `v774.js` 29 / `cycle5.js` 60 / `v773.js` 43 全过。 || v778 | **周期系统分析：收起态给出逐档明细 + 全模块文案去版本号、改写为专业表述**。①**收起态**——此前收起只剩一行「模块名 + 阶段 + 得分 + 证据%」（约 52px），等于把结论压成一个没有上下文的数字：「3.6 分」本身不说明任何事，三档全是轻权重的行为性信号、和 T1 已经亮了一条是完全不同的处境。新增 `cycDigestHTML(p)` 渲染在 `<summary>` 内的三张维度卡（T1 决定性 / T2 结构性 / T3 行为性），每张给出：每条权重、状态分段条（已触发红 / 观察中黄 / 未出现绿 / 未填灰，按条数占比分段）、逐状态计数、以及**该档对总分的贡献**「贡献 X / 已核实 Y 分」——三档贡献之和就是总分的分子，读者可以自己把 3.6 分拆回来源。下方脚注带临界提示与得分走向。`cyclePhase()` 因此新增 `tiers` 逐档明细（原 `cnt` 的 `[lit, 总数]` 不够用），`[open]` 时整块 `display:none`——展开后下方有完整清单，留着就是重复信息。实测收起态 52→193px（手机 830px 单列），展开态 4730px 不变。②**去版本号**——用户可见文案里唯一一处是「利好失效」基线备注结尾的「（vNNN 曾误记已触发…vNNN 更正）」，已删；模块内 9 处开发者注释的 `vNNN` 标号也一并去掉，**理由本身全部保留**（如「早期版本的三个缺陷」「若分母是判据满分，每加一条判据显示的分数就换一把尺子」），去掉的只是对读者毫无意义的版本编号。顺带修掉 5 处**渲染进 HTML 却写成 markdown 星号**的强调（`**上限**`/`**方向是加息**` 等），它们此前在卡片上原样显示成星号，现改为 `<b>`。③**文案专业化**——规则行重排为「评分 / 权重按决定性划分 / 阶段分档 / 完整度门控」四段带小标题的结构（原先是一整段连排）；完整度定义、过期汇总、适用范围、距下一阶段说明、得分走向空态、临界提示、一键载入说明全部从口语（「别把它当成一个稳的结论」「还只有 N 个读数」）改为书面表述（「不宜当作稳定结论」「当前仅有 N 个读数」）；阶段定义的行标签「清单长相」→「清单形态」。**判定结果与算法完全未变**：满分仍 21、得分仍 3.6/10 → 第 4 阶段·金融化、完整度 100%、仍在临界（距 3.5 线 0.15 分）——本版只动展示与文案。Playwright 回归（新增 `v778.js` 40 项）：三档卡的权重/计数/分段条宽度（T2 已触发段 2/5=40%）/贡献值均与手算一致（T1 0/6 · T2 6/10 · T3 1.7/5，合计 7.67÷21×10=3.6）、收起可见与展开隐藏、收起高度 193px、全文遍历确认 0 处 `vNNN` 与 0 处未渲染星号、7 项新文案断言、手机端单列且无横向溢出；`cycle2` 43 / `cycle3` 21 / `cycle5` 60 / `v768ui` 12 / `v769` 20 / `v770` 24 / `v771` 25 / `v773` 43 / `v774` 29 / `v775` 18 / `v776` 46 / `eqafter` 20 全过（cycle3/v769/v770 共 8 条断言是按旧措辞逐字匹配的，已按新文案更新——所测的实质内容仍在卡片上，只是换了说法）。 |
| v779 | **收起态逐档摘要改版：三张大卡片 → 一行一维度**。v778 那版把三档画成三张等宽大卡，主体是一条**按条数比例**分段的进度条——它既不是得分也不是状态，只是「五条里有两条」，占了最大面积却信息量最低，配上「贡献 6 / 已核实 10 分」这种工程口径的措辞，整体读起来是空的。改为**每档一行**（`.cyc-dim` 由 grid 卡片改为 flex 行）：左侧 T 徽章 + <b>维度名称</b>（决定性 / 结构性 / 行为性）+ 副标题写明这一档到底在测什么（capex 指引与半导体订单 / 融资结构、资金成本与投入产出 / 会计手法与市场行为，此前只有抽象的三个词）；右侧三段固定宽度对齐——**逐条状态点**（一条判据一个圆点，红已触发 / 黄观察中 / 绿未出现 / 空心未填，`title` 带「判据名：状态」），**状态词徽章**（干净 / 初现 / 承压 / 全面触发 / 未核实），**该档得分**「6/10」。状态词刻意按 `该档已得分 ÷ 该档已核实满分` 分档而不是按条数——一条 T1 亮起和三条 T3 亮起在条数上都是「有几条」，在含义上完全不是一回事。`cyclePhase()` 的 `tiers` 新增 `items[]`（逐条 `{zh, st}`）供状态点使用。手机端：隐藏副标题保证「名称 + 状态 + 得分」同一行，状态点换到第二行整行铺开。实测收起态 243px（v778 为 193px、原始单行约 52px），展开态 4730px 不变，桌面手机均无横向溢出。**算法与判定结果仍未变**：满分 21、3.6/10 → 第 4 阶段·金融化、完整度 100%、距 3.5 线 0.15 分。Playwright（`v778.js` 扩到 45 项）：点数 = 判据条数 2/5/5、T2 两红三黄、点上 title 含判据名与状态、三档状态词与得分（0/6 干净 · 6/10 承压 · 1.7/5 承压）逐项对手算、三档之和 7.67÷21×10 = 3.6 与总分一致、手机端副标题隐藏且状态点确实换行。 |
| v780 | **收起态改为逐条列出全部 12 条判据**（用户：「12 个维度每个都写出来并且给出对应的未出现这几个标签，因为是收起的，考虑如何精简化设计」）。v779 把 12 条压成三档、每档只给一个聚合状态词，等于又回到「看不见具体是谁亮了」；现在 **12 条判据一条一个 chip 全部列出**，每条自带 T 档位 + 判据名 + 状态标签（未出现 / 观察中 / 已触发 / 未填），收起状态下不展开就能看清这 3.6 分具体由哪几条构成。**为收起态做的精简**：①`CYCLE_ITEMS` 每条新增 `short` 短名（「AI capex 债务融资占比」→「债务融资占比」、「宽度背离（等权 vs 市值加权）」→「宽度背离」等），完整名退到 `title`；②chip 结构压到三段（`i` 档位 9px mono / `b` 名称 11.5px 带 ellipsis / `u` 状态 pill 9.5px），单条高约 26px；③桌面 4 列 × 3 行、手机 2 列 × 6 行的紧凑网格；④**按档位排序**（2×T1 → 5×T2 → 5×T3），阅读顺序与权重一致，不需要再画分组标题；⑤v779 的三档得分压成标题行右侧一行小计「T1 0/6 · T2 6/10 · T3 1.7/5」——档位信息不丢，但只占一行。脚注保留临界提示与得分走向。实测收起态桌面 227px / 手机 360px（v779 为 243px，v778 三张大卡为 193px 但只有三个聚合数），展开态 4730px 不变，两档均无横向溢出。**算法与判定结果仍未变**：满分 21、3.6/10 → 第 4 阶段·金融化、完整度 100%、距 3.5 线 0.15 分。Playwright（`v778.js` 扩到 50 项）：12 条逐条断言状态（3 已触发 / 5 观察中 / 4 未出现，与展开后的清单同源）、档位排序、短名与 `title` 完整名对应、小计文案与手算一致且三档之和即总分分子、桌面 3 行与手机 6 行的实测行数、收起高度上限。 |
...
| v786 | **模拟期权数据整体删除**（用户：「期权页模拟的信息都可以删除」）。v785 只拆了 UI、把数据留着；本版按要求把它删干净：`window.SIM_OPTIONS` 全局变量（`data.js`）、`saveLocalOnly`/`loadFromStorage` 里的 `trendo_v4_sim_options` 读写、`_buildSyncPayload` 与 `applyCloudData` 里的 `simOptions` 字段、`syncOnStartup` 的 `localTotal` 计数、以及 `api/data.js` 的种子数据全部移除。**云端那份怎么消失的**：同步 POST 是**整块覆盖**而不是字段合并，所以载荷里不再带 `simOptions` 之后，下一次推送就会把云端 blob 里的这个字段一并替换掉；本地残留则由脚本加载时的一次性 `localStorage.removeItem("trendo_v4_sim_options")` 清掉。顺带把卖出弹窗里 14 处 `simOptionsSym`/`simOptionsStrat` 改名为 `optSellSym`/`optSellStrat`（期权页已经没有「模拟」这回事，留着这个名字会误导），并删掉声明后从未使用的 `simOptionsVisible`。**这是不可逆的**：模拟期权的历史记录不再存在于本地、云端与代码任何一处。Playwright 回归（新增 `v786.js` 21 项）：带着残留数据打开后 localStorage 键被删、`window.SIM_OPTIONS` 已 undefined、实盘期权与卡片不受影响、推送载荷确认不含 `simOptions` 且仍带 `realOptions`；源码层全站清零（含注释外的所有引用、改名后的变量、`localTotal` 只数实盘）；全新设备从**仍带旧 `simOptions` 的云端 blob** 引导时正确无视该字段、不写回本地且无报错。（测试踩到与 v784 同一个坑：切后台走 `navigator.sendBeacon`，Playwright 的 route 拦不到，需在页面里替换它才能断言载荷。） |
| v787 | **周期系统分析：基线「已载入」状态 + 清单按重要程度排序**（两条用户要求）。**①已载入状态**——此前那个「按今天日期载入基线」按钮是个纯动作，点完页面上没有任何地方说明「现在这张表跟基线一不一致」，用户只能自己逐条回忆有没有改过。新增 `cycBaselineStatus()`：逐条比对当前状态与基线推导出的状态（数字项**连数值一起比**——政策利率填 3.00 与 4.00 同为「观察中」，但那不是同一份读数），返回相同/不同条数与最近一次复核日期。按钮旁渲染 `.cyc-apply-state`——全部一致时显示绿色「✓ 已载入 · 复核日期 YYYY-MM-DD」，否则黄色「N/11 条与基线不同」。**不比备注**：备注是用户自己的，载入基线本来就不覆盖它。**按钮不禁用**（已载入时再点仍有意义——把复核日期重新记为今天，完整度回 100%、过期提示清零），改为按钮文案变为「重新按今天日期确认」+ 转灰描边，确认框也换成「只改日期、不改状态」的说法；说明文字分两种情形：已载入时点明「再点只改复核日期」，不一致时点明「既可能是你自己改过，也可能是基线本身更新了」。**②清单按档位排序**——`.cyc-list` 一直按 `CYCLE_ITEMS` 的**声明顺序**渲染（capex(T1) → 折旧(T3) → 半导体(T1) → …），而收起态摘要早就按 `sort((a,b) => a.tier - b.tier)` 排过，两处同一份清单顺序不同；且声明顺序里一条 T3 夹在两条 T1 中间，读下来完全看不出权重梯度。现改为 T1 → T2 → T3 三组，每组带 `.cyc-group` 标题（档位徽章 + 决定性/结构性/行为性 + 该档在测什么 + 「权重 N 分 · M 条」），与收起态摘要同序。Playwright 回归（新增 `v787.js` 42 项）：默认种子下显示「已载入 · 2026-09-17」且按钮转灰改文案；改一条状态 → 「1/11 条与基线不同」，再改一个数字项数值（状态不变）→ 「2/11」；点载入后回到已载入且复核日期为今天、恢复动作写进变更记录；已载入时再点确认框说明「不改变任何状态」、复核日期更新为今天且**不产生留痕噪音**；三个分组顺序与权重文案、12 条判据排序 T1,T1,T2×5,T3×5、每条都落在自己分组下、折旧年限（声明顺序第 2）被正确推到 T3 组；桌面 1400px 与手机 390px 均无横向溢出。 |
| v788 | **已平仓表格新增「平仓日期」列（现持仓 + 模拟仓）+ 修掉 `fmt.date` 的差一天**（用户：「已平仓表格模块也给出平仓的日期」）。**①新增列**——已平仓页签此前能**按**平仓时间排序（`CLOSED_SORTS` 里一直有「平仓时间」、切过去的默认排序就是它），却**看不到**这个日期，只能开抽屉一笔笔看。`COLS` 新增 `{ id:"closedAt", label:"平仓日期", openHide:true }`，插在「数量」与「盈亏」之间；`openHide` 是 `closedHide` 的另一侧（止损止盈在已平仓没意义，平仓日期在持仓中同样没意义），`visTableCols` 一处改动同时覆盖现持仓与模拟仓两张表。排序**零新增代码**：`CLOSED_KEYFN.closedAt` 早就存在，点表头直接生效。分批出场的合并行取最后一次出场日（`mergeClosedForDisplay` 的既有口径），`title` 补「入场 → 平仓 · N 天」整段区间；没有 `closedAt` 的脏记录降级为 `—` 而不是报错。卡片视图同步（同一个模块的另一种视图，信息缺口是同一个）：已平仓卡片的天数行前缀「Sep 20 平仓 · 」，持仓中不变。设置里的列选择器给 `openHide`/`closedHide` 的列各加一个「(仅已平仓)」「(仅持仓中)」小标注，否则勾了没反应会让人以为坏了。手机端已平仓页签 `min-width` 640→730px（`.closed-tab` 类由`renderTable`/`renderSimTable` 切换），多出来的那一列不至于被挤窄。**②顺带修掉一个存在已久的差一天 bug**：`fmt.date` 一直是 `new Date(iso)` 直解，而 `"YYYY-MM-DD"` 会被按 **UTC** 解析，再用本地时区格式化时，美洲负时区整体回退一天——**入场 09-20 的仓位一直显示成 "Sep 19"**（Node 实测确认）。这与 v584 在财报日历上踩的是同一个坑，改为逐字段解析本地自然日，与 `parseLocalDate` 同一套口径。`fmt.date` 的 **11 个调用点全部受益**：抽屉的入场/平仓日期、执行记录、Journal 出场日期、分享卡片的区间、以及本版新增的这一列。Playwright 回归（新增 `v788.js` 54 项，整套在 `TZ=America/New_York` 下跑）：先在 Node 侧确认旧写法确实报 Sep 19，再断言页面显示 Sep 20；持仓中页签无此列且列集不变、已平仓页签列序正确；分批两次出场取 09-18、无 closedAt 降级为 `—` 且不挂 title、title 含完整区间与天数；默认降序 + 点表头翻升降各一次；卡片视图三种记录的文案；列选择器标注与取消勾选后该列消失；手机端 390px 两个页签的 min-width 实测 730/640px 且页面无横向溢出。 |
| v789 | **卡片模式：日期挪到卡片上方，开仓/平仓同一坑位同一样式**（用户：「把出场日期放在卡片的上方…和正在持仓的相同展示」）。v788 把平仓日期加进卡片时图省事塞进了中段 `hc-days` 的文字前缀（「Sep 20 平仓 · 54天」），读起来像临时补丁，且持仓中卡片完全没有对应的日期展示、两种状态不对称。改为在卡片最上方——`.tk .meta`（股票代码/名称所在的头部区块）新增一行 `.hc-meta-date`，**开仓与平仓复用同一个坑位、同一套 CSS**：持仓中显示「入场 Aug 03」，已平仓显示「平仓 Sep 20」，无 closedAt 的旧记录留空、不报错。`hc-days` 相应改回只显示纯天数（如「54天」「28天 · 2次出场」），不再带日期前缀。**这是本版唯一改动**——列表视图（v788 新增的「平仓日期」表格列）与排序逻辑均未受影响。Playwright 回归（新增 `v789.js` 26 项）：持仓中/已平仓两种状态下日期文案、位置（确认在 `hc-top` 内而非 `hc-pnl-row` 中段）、class 一致性、无日期时留空、现持仓与模拟仓两张表、手机端 390px 均验证；`v788.js` 的 2 处旧断言按新结构更新（原断言的是「日期塞进 hc-days」这个被本版替换掉的设计，不是回归）；`closedsort.js` 25 项、`v787.js` 42 项保持通过。 |
| v790 | **卡片模式：已平仓也按日期分组、加小节标题——复用持仓中早就有的那一套**（用户配图指出「是这种样式，日期在卡片的上方」，图中「Aug 28」是卡片外面、独立于卡片的一行小节标题，不是 v789 加在卡片内部的那行）。排查发现这个分组机制在持仓中页签**早就存在**——`renderHoldingsCards`/`renderSimHoldingsCards`一直会把持仓按 `entry` 日期分组、组间插入 `.hc-date-hdr`（如「Aug 28」），组按日期降序排列——只是「已平仓」分支从来是一串不分组的裸卡片，两个页签用了两套不同的展示逻辑。提炼出共用的 `dateGroupedCardsHTML(rows, dateOf, cardHTML)`，持仓中继续按 `entry` 分组，**已平仓现在按 `closedAt` 分组**，同一天平仓的多笔合并在同一个日期标题下；无 `closedAt` 的旧记录归入「—」组，不报错。现持仓与模拟仓两处调用点都替换为这个共用函数，四份几乎相同的分组代码消成一份。**v789 加在卡片内部的「入场/平仓 日期」那一行原样保留**——两层信息并存：卡片外的分组标题回答「这一批是哪天的」，卡片内的日期行回答「这一笔具体是哪天的」，同一天分组里有多笔时后者仍有区分度。**自测抓到一个真冲突**：已平仓页签自带排序下拉（收益率/R倍数/持仓天数…），如果不分青红皂白一律按 closedAt 分组，选「按收益率排序」时会被日期分组打断——组间顺序仍按日期降序，用户要的「整段按收益率排名」被悄悄拆碎在各个日期小组里。修法：分组只在 `sortKey`（现持仓）/`simSortKey`（模拟仓）**等于 "closedAt"**（默认排序，也是切到已平仓页签落地时的状态）才生效；选了其他排序字段就退回不分组的整段列表，原有的「按此字段整体排名」如实兑现。持仓中页签的分组则保持无条件生效——它此前就是这样、且没有暴露排序控件与之冲突。Playwright 回归（新增 `v790.js` 24 项）：持仓中分组不受影响、已平仓默认排序下正确按 closedAt 分组（同日合并/无日期归入「—」/最新日期组在最上面）、卡片内部的 `.hc-meta-date` 保留、**切到「盈亏金额」排序后分组让路、整段按金额排名**、现持仓与模拟仓、手机端 390px 均验证；`v789.js` 26 / `v788.js` 56 / `closedsort.js` 25 / `v787.js` 42 保持通过（`closedsort.js` 首轮测出分组与排序冲突，修完后转绿）。 |
| v791 | **卡片模式：删除卡片内部的入场/平仓日期行**（用户：「现持仓和模拟仓卡片里的入场日期可以删除，因为上方写了已经」）。v789 把日期加进卡片内部（`.tk .meta` 里的 `.hc-meta-date`，「入场 Aug 28」/「平仓 Sep 20」），v790 又在卡片外面补了按日期分组的小节标题（`.hc-date-hdr`，同样写「Aug 28」）——两处显示同一个日期，v790 之后卡片内部那行就是纯冗余。整块删除 `holdingCard()` 里的 `.hc-meta-date`（JS 与配套 CSS），`.meta` 恢复成只有代码+名称两行。**分组标题（v790）与排序让路规则不受影响**——日期依旧只在卡片外显示一次。Playwright 回归（新增 `v791.js` 20 项）：源码层确认 `hc-meta-date` 在 desk.js/index.html 中彻底清零、`dateGroupedCardsHTML` 分组函数仍在；持仓中与已平仓两种状态下卡片外的分组标题（Aug 28/Sep 20）保持显示、卡片内部确认无日期行、`.meta` 只剩 `sym`/`nm` 两个子元素；现持仓与模拟仓、手机端 390px 均验证。`v788.js`/`v790.js` 里断言「卡片内部保留日期」的旧用例已按新设计改为断言「卡片内部不再重复日期」，`v789.js` 整份测的是本版删除的功能，作废不再纳入回归（历史记录保留在仓库里）。 |
| v792 | **抽屉开着时跟随行情实时刷新**（用户问「盈利保护是价格跌破保护价时展示，价格重回之上应该消失吧」）。**先确认：逻辑本来就是这样的**——`isBreach` 在 `ppBlockHTML()` 里每次渲染都按 `h.last < h.ppPrice` 现算，不是一次性锁死的状态；关掉抽屉重开，「已触及」会自己回到「已激活」、⚠️提示消失、保护价标签从 ▼ 翻回 ▲、轴填充回 safe 色——Playwright 实测确认。**但确实有个真 bug**：`fetchPrices()` 只重渲染表格/总结/月度/Analytics，**从不碰打开着的抽屉**，所以抽屉是「打开那一刻的快照」——实测价格从 115 涨到 130 后，表格已经是 $130.00，抽屉里仍然停在 $115.00、仍然挂着「已跌破保护价」的警告。盈利保护受影响最明显，因为它的提示本来就该随价格来去，卡住之后看起来像个撤不掉的警告。**修法**：新增 `syncDrawerLive()` + `_drawerLive`（在 `openDrawer`/`openSimDrawer` 记下当前这笔、`closeDrawer` 清空；已平仓不记——没有实时价这回事），在 `fetchPrices()` 的重渲染块末尾调用。**只就地更新跟价格有关的几块**（现价/涨幅/盈亏/R/level bar/`#pp-block-wrap` 整块重渲染），**不整块重建抽屉**——`openDrawer()` 会重建 innerHTML 并重新 wire，30 秒一次那么干会把用户正在编辑的输入框、BX 的入场/实时页签、滚动位置全冲掉。复用的是抽屉编辑里早已存在、重复了三处的那段 hero+levelBar 就地更新写法。**正在输入 ATR 时跳过 PP 块重渲染**（`document.activeElement?.id !== "drawer-pp-atr"`），否则光标和没填完的数字会被冲掉。Playwright 回归（新增 `v792.js` 31 项）：现持仓抽屉开着不动、价格 115→130 后徽章回「已激活」、⚠️消失、标签 ▼→▲、填充 breach→safe、效率 chip 重算为 75%、现价与涨幅同步；**再跌回 115 提示重新出现**（双向跟随，不是一次性）；模拟仓同样生效；正在输入的 ATR「7.7」与光标都保住而其余部分照常更新；已平仓抽屉无 PP 块且跑一次轮询不报错；手机端一致无溢出。**测试踩到的坑**：Playwright 按「后注册优先」匹配 route，catch-all `**/api/**` 必须先注册，否则会吃掉 `**/api/quote*`——首轮因此误判「表格也没更新」，差点把结论写反。 |
| v793 | **市场日报改为「每天北京 07:30 定时抓取 + 手动 ↻ 无上限」，展开不再触发生成**（用户明确要求这套新规则，取代 v781/v784/v785 那三条客户端自发触发逻辑）。此前有三条路径会调用 Claude 生成一次：展开卡片（60秒内去重）、进 Market 页时若缓存跨自然日（每天一次）、手动 ↻（无限制）——前两条现已**整体删除**：`_briefExpandShouldFetch`/`_briefLastExpandFetch`/`_briefStale` 三个函数全部去掉，展开/收起卡片和进入 Market 页都只是「查看」，不再产生任何请求；只剩两条真正会调用 Claude 的路径：①`vercel.json` 的 Cron（改期），②手动 ↻（本来就是每次强制重生成，天然无上限，未改）。**缓存槽从「北京 09:30/21:30 两档」改为「北京 07:30 为界、一天一槽」**——与新 cron 时刻对齐：新增 `_briefSlotKey()`（07:30 之前仍算「昨天」那个槽，避免 cron 还没跑就被判过期），`_saveBrief`/`_loadBrief` 加 `slotFn` 参数（默认仍是旧的 `_bjSlotKey`，供持仓分析卡片继续用两档缓存，未改）；服务端 `api/market-summary.js` 的 `bjSlotKey()` 同步改为按 07:30 锚定，Redis TTL 43200→93600 秒（~26h，留够跨日边界的余量）。`vercel.json` 的 cron 从 `30 1 * * *`（UTC，=北京 09:30）改为 `30 23 * * *`（UTC 23:30 = 北京次日 07:30，北京无夏令时，偏移恒定）。**「展示的对应时间就是这个」如何达成**：客户端不再自己驱动更新，`initMarketBriefCard` 改为——本地有缓存就先原样渲染（新增 `_loadBriefRaw`，不做槽位匹配，哪怕是隔天的旧数据也先摆出来不空一下）；若缓存的槽位不是当前槽，才发一次 **非强制**（`force` 不设）的探测请求——这次请求命中 Redis 就直接返回 cron 生成的最新版（不烧 Claude），只有 Redis 也没有（cron 还没跑过/失败）时才会退化成现场生成一次，作为合理兜底；时间戳因此天然等于 cron 实际生成的那一刻。**手动刷新无上限**：`.brief-refresh` 点击一直是 `fetchMarketBrief(true, mktCtx)`，跟之前一样没有任何去重/冷却，确认符合要求、未作改动。缓存破坏 v792→v793。Playwright 回归（新增 `v793.js` 17 项）：无缓存只显示生成按钮且不自动调用；有缓存先渲染（不管是否要后台换新）；缓存跨槽时恰好发 1 次非强制请求且换新成功；展开/收起卡片确认 0 次新请求；连点 4 次 ↻ 确认各自触发 1 次 `force=1` 请求（无上限）；`vercel.json`/`market-summary.js` 源码层确认新 cron 时间与 07:30 锚点逻辑；`desk.js` 源码层确认三个旧触发函数已彻底删除。`v781.js`（测的正是被本版删除的「展开即重新生成」「跨天自动重生成」两条旧路径）按预期在场景 1/2/4/5 转红——**整套作废退出回归集**，不再修补（历史记录保留在仓库里，与 v791 对 v789.js 的处理同例）；其余全部无关套件（v784/v785/v792/v790/v791/closedsort/verdict/eqafter/cycle5/v787/v788/v783/v776/v778）复跑全绿，确认无连带回归。 |
| v794 | **周期系统分析：新增第 12 条「AI 资本开支参与广度」替换掉「新估值指标」，档位从 T3(1分) 升为 T2(2分)**（用户讨论后明确要求，并追问"重要程度和计算方式是否要更新"）。**①为什么选它替换、又为什么升档**——`new_metric`（新估值指标）自 v766 起一直是"未出现"，边际价值最低，是 12 条里最适合让位的一条；新条目测的是"头部四五家之外还有没有新买家愿意加码"，这是**可核查的事实**（谁在真金白银投钱），不是能被情绪解释掉的行为信号，因此归入 **T2 结构性**而非沿用被替换条目的 T3——这是本版的核心判断，不是简单同位替换。权重分布因此从 2/5/5（满分21）变为 **2/6/4（满分22）**。**②改权重必须重验**（v761 立的规矩，`enum794.js` 穷举 3¹²=531,441 种组合复核）：新分布下阶段占比与得分分位数几乎不变（第4阶段 22.5%→23.2%，p50 4.44→4.39），**现有 6.0/3.5/1.5 分档线无需调整**。**③条目设计**：`capex_breadth`，三行结构「测什么/去哪看/怎么判」——去哪看指向二线云厂商/neocloud（CoreWeave、Nebius）财报与融资渠道、主权财富基金 AI 基建配置比例、企业级 IT capex 调查；`warn` 明确"靠借头部的钱买头部芯片"这类关联融资算退场信号、不算广度扩大，避免跟 `circular` 那条的判据混淆。**④联网核实的基线（观察中）**：两条相反证据并存——主权财富基金对基建的平均配置从 2022 年约 5% 升到 2025 年约 9%、海湾主权基金是最积极的追加者，广度确实在扩大；但二线云厂商几乎全靠举债扩张、没有头部厂商的现金流护垫，且头部四五家 2026 年资本开支合计约 6,600–6,900 亿美元，仍是其余所有参与者总和的数量级压制——广度在扩、根基却脆弱，故记观察中。**⑤代入当前完整基线**：得分从 3.6/10 变为 **3.7/10**（8.333÷22），仍是**第 4 阶段·金融化**，结论未变。**⑥意外验证到一个已有机制按预期工作**：`cycleSeed()` 本就设计成"逐条补齐"而非"整表补齐一次"（v766 起的设计，为的是老用户也能吃到新增判据）——本版新增条目后测试证实，已经存过数据的老用户设备打开后会自动补上这一条为基线状态，不需要额外写迁移代码。Playwright 回归（新增 `v794.js` 20 项）：12 条判据数不变、参与广度出现且档位 T2、旧条目彻底消失、核实说明含主权基金与二线厂商两组证据、基线默认「观察中」、T1/T2/T3 计数精确为 2/6/4、分档线源码层确认未被误改、完整度 100%、手机端一致。规模较大的两个存量套件因硬编码了旧权重下的具体分值被订正（非退役，逻辑本身仍有效，只是数字随新权重变了）：`v769.js`（10 分制主数字 3.6→3.7、分布 2/5/5→2/6/4、宽度背离场景 4.1→4.2 等 6 处）、`v771.js`（距下一阶段的分差 2.35→2.21 分 / 0.15→0.29 分、tiny 场景补上 `capex_breadth` 的显式 unset 避免被 `cycleSeed()` 自动补位抢走贪心候选、高分场景 7.1→7.2 等）；`v787.js` 两处分组标题的条数文案（"5 条"→"6/4 条"）与排序断言同步更新。全部改动均先跑真实页面核对新数值再回填进断言，未凭手算数字直接下笔。`cycle2` 43 / `cycle3` 21 / `cycle5` 60 / `v768ui` 12 / `v770` 24 / `v773` 43 / `v776` 46 / `v778` 50 / `v781`(已作废) / `v782` 28 / `v783` 24 / `v784` 37 / `v785` 38 / `v786` 21 / `v788` 56 / `v790` 24 / `v791` 20 / `v792` 31 / `v793` 17 / `closedsort` 25 / `verdict` 21 / `eqafter` 22 / `v774` 29 / `v775` 18 全部复跑绿灯。 |
| v795 | **周期系统分析得分走向：区分「标尺调整」与「证据变化」**（用户追问"分数这么越来越多了呢？"，复盘发现这套评分历史上历次分数变化混杂了两类完全不同的原因——真正的证据变化几乎没怎么推动过分数，推动分数的主要是权重结构本身在变：v768→v769 同一批 12 条判据状态一个字没改，仅因把档位从"2/8/2"重分配成"2/5/5"，判定就从第3阶段跳到第4阶段；这次的 v793→v794（new_metric T3→capex_breadth T2）同理，3.6→3.7 也是纯权重再分配。`.cyc-hist` 走向曲线此前把这两类变化画在同一条线上，回看历史容易把"重新称重"误读成"周期风险在上升"）。**修法**：`cyclePhase()` 新增返回 `wAll`——12 条判据**不论填没填**的权重总和，只在新增/删除判据或给已有判据重新分档时才会变，判据状态本身变化不会碰它，是这张表的"量尺签名"；`cycleSnapshot(p)` 把 `wa: p.wAll`一并存进每条历史快照。`cycHistFlag()` 逐条比对相邻快照的 `wa`，不同就标记这一步发生过"标尺调整"；`cycHistHTML()` 据此在对应行加 `⚖ 标尺调整` 标签（title 写出"满分从 X 变为 Y"），窗口内若含任意一次标尺调整，顶部走向摘要下方追加黄色 caveat 提示"这段区间发生过评分口径调整，走向不能整段归因于证据变化"。**老记录兼容**：v795 之前存的快照没有 `wa` 字段，`cycHistFlag()` 对缺字段的相邻对一律不判定（宁可漏报也不误报），不会把历史数据错误地标成"标尺调整"。**判定结果与算法完全未变**：这次纯粹是给已有的得分走向面板加一层归因标注，不影响当前 3.7/10、第4阶段·金融化 的判定。Playwright 回归（新增 `v795.js` 16 项）：wa 相同的相邻快照不标记、wa 不同的恰好标记 1 处且 title 含"满分从 21 变为 22"、caveat 仅在窗口含标尺调整时出现、老记录缺 wa 字段不误报（含"一侧有 wa 一侧没有"的边界）、三条混合场景里证据变化行不标记而标尺调整行标记、手机端一致无溢出、源码层确认当前 2/6/4 结构下 wAll=22。`cycle2` 43 / `cycle3` 21 / `cycle5` 60 / `v768ui` 12 / `v769` 20 / `v770` 24 / `v771` 25 / `v773` 43 / `v794` 20 复跑全绿——`wAll` 是新增返回字段，不影响任何既有断言。 |
...
| v799 | **Market 页新增「建议走向」卡片（`mkAdviceHTML`）——综合建议的逐日回放色带**（用户讨论后选定做成独立模块，而不是并进阶段周期卡）。**①关键发现：不用攒快照，数据一直都在**——我最初设想的是"每天存一条慢慢攒"，那方案有两个硬伤（只有上线后才有数据、用户不开 app 的那些天断档）。查下来发现 `combineAxes` 的每个输入都能逐日重算：方向轴/风险容量轴本来就在 `buildPhaseHistory` 里逐日算了，RSI、`vixTrend`、`vix60Max` 都是 VOO/VIX 历史的派生值（此前只是**只算了最新一天**），**唯一看似缺口的 FGI 其实也在手上**——`api/feargreed.js` 请求的 CNN 端点叫 `graphdata`，它本来就是给官网那条一年期曲线供数的，响应里一直带着 `fear_and_greed_historical.data`，而代理只取了今天和昨天两个数、把整段历史丢掉了。所以零新数据源、零新 serverless 函数（Hobby 那 12 个的限制不受影响）、零额外请求。**②新增 `calcRSISeries`**（与 `calcEMASeries` 对 `calcEMA` 同样的关系）：**刻意逐点复刻 `calcRSI`，包括它"seed 之后从 period+1 起步、跳过 `closes[period]` 那根 bar"这个跟教科书 RSI 差一根 bar 的细节**——回放的最后一格必须等于卡片上现在显示的 RSI，两套数字对不上是用户看得见的 bug，跟教科书差一根 bar 是看不见的、而且全站一致。测试逐点比对 55 个采样点确认零偏差。**③`combineAxes` 新增稳定 `id`**（defense/trim/cooldown/accumulate/scale/hold/normal）供色带分段——`headline` 是给人读的文案、v538→v560 之间改过好几版，拿它当分段键会让文案一改历史就碎成两段。**④刻意不做的三件事，都如实印在卡片上**：用**今天的规则**跑历史，所以它是"当时若用现在这套规则会给什么建议"、**不是当时页面上真的显示过什么**；覆盖范围 = FGI 历史（约一年）∩ EMA200 预热（200 个交易日），按**实际跨度**报数而不是笼统写"近一年"；FGI/VIX 缺口用**前值**补（那是它们当天的真实状态，不引入 look-ahead），序列开始之前没有前值可补的那些天直接不回放，FGI 整个拿不到就明说"无法回放"并列出缺什么——**绝不用今天的 FGI 回填历史**，那会造出一条看着完整、实则虚构的线。**⑤关于抖动（`buildPhaseHistory` 顶部注释早就否决过"用综合状态画色带"）**：那个否决针对的是用它**定义市场阶段的边界**，至今成立，所以阶段周期卡仍然只由方向轴定义；这张卡回答的是另一个问题——这条建议是什么时候变成现在这样的，抖动因此是要如实呈现的属性、不是要消除的东西。色带**逐日照画不做平滑**，只有转换列表按 `ADVICE_MIN_SEG`(3) 折叠短段，且把折叠掉的次数直接报在摘要行（"146 次切换 · 135 次短于 3 日未列出"）。外网在本环境不可达、拿不到真实数据实测碎片化程度，所以改为**把实际次数印在卡片上让真实数据自己回答**。**压力实测**（病态构造：方向/风险轴全程不动，只让 FGI 精确地每天在 59/61 之间跳、且价格震荡把 RSI 压在中性区使 FGI 成为唯一翻转来源）：201 个交易日 → 147 段、146 次切换，而切换列表仍只列 11 条、卡片高度 302px 没被撑爆，图例的占比（保持持仓 64% · 正常配置 20% · 分批参与 16%）在色带碎成条纹时接管了"主要待在哪个状态"这个问题——最坏情况下卡片依然可读。**⑥复用**：与 `buildPhaseHistory` 同构的返回结构，因此直接套用 `.mkp-bars`/`.mkp-ribbon`/`.mkp-seg`/`.mkp-ticks`/`.mkp-legend`/`.mkp-fold`/`.mkp-tr` 整套 CSS，零新增布局；折叠状态走 `wireMarketFolds` 已有的通用机制（`data-mkp-fold="adv"` → `trendo_mkp_adv_open`），不用再写一遍。图例顺带带上每态天数与占比——色带看得出形状、看不出占比，而占比恰恰是最容易被问到的。Playwright 回归（新增 `v799.js` 54 项）：`calcRSISeries` 逐点一致性、色带分段/各段天数之和 = 头部总数/不超过 252 天窗口、图例占比合计 100%、**回放确实用当天 FGI 而非今天的值回填**（今天传 fg=50，早期 FGI=80 的那段仍正确回放出"止盈"）、切换列表不含短于 3 日的段且每条都指名翻转的轴、折叠状态落盘且跨页重建后保持、FGI 缺失时优雅降级且不连累综合建议横幅与阶段周期色带、手机端 390px 无溢出、抖动压力测试 7 项、源码层接线。`v782` 27 / `v793` 17 / `v795` 16 / `v796` 14 / `v798` 20 / `v787` 42 / `v792` 32 / `v794` 20 复跑全绿。**顺带订正一个与本版无关的陈旧套件**：`cycle5.js` 跑出 7 项红灯，用 `git stash` 对照确认**改动前同样是 53/7**——是 v794 把 `new_metric`(T3/1分/未出现) 换成 `capex_breadth`(T2/2分/观察中)、满分 21→22、得分 3.6→3.7 时只更新了 `v769`/`v771`/`v787`、漏掉了 cycle5，产品是对的、断言过期了。按新口径订正（权重和 7.67→8.33、分母 21→22、3.6→3.7、宽度背离点亮 4.1→4.2），另有一条连带变化值得记下：**换权重后得分 3.79 距 3.5 线 0.29 分，已超过 ±0.2 的临界阈值，因此不再标临界提示**——v769 起那个"阶段贴着分档线"的状态在 v794 就已经解除了，只是没人注意到。订正后 60/0，与 v794 记录的项数一致。 |
| v800 | **建议走向卡：加状态说明列表 + 窗口 252→200 个交易日 + 补齐与下方模块的间距**（用户反馈：「每个状态是啥意思不太懂，并且可以使用200个交易日的来展示，不然信息太多手机端的也太密，这里和下方模块的间隔给出」）。**①状态说明**——v799 的图例只有颜色圆点+天数+占比，"防守""止盈""兑现"这些标题此前只在色带 hover 的 `title` 里才有解释，手机端根本摸不到。新增 `.mkp-advice-defs`：`buildAdviceHistory` 的 `day` 对象里补一个 `detail` 字段（取自 `combineAxes` 返回值，跟横幅、色带 tooltip 用的是同一份措辞，不是另造文案），渲染时按 tally 分组、**只列这段窗口里实际出现过的状态**（不是把全部 7 种可能状态列一遍——没出现过的跟当前无关，列出来只增加要读的行数），每条给「名称 + state（为什么）+ detail（怎么做）」三段。**②窗口收窄**——`ADVICE_SESSIONS` 252→200，不再跟阶段周期同窗口：那张卡只有 3 态、段落天生少，这张卡最多 7 态、压力测试一年能碎到 147 段，窗口越长手机端色带/图例/说明列表就越密；200 个交易日（约 10 个月）是"看得出走向"与"手机端读得下去"之间的折中。空态提示与底部 `.mkp-note` 同步改为报出 `${ADVICE_SESSIONS}` 个交易日（约几个月），不再是写死的数字。**③模块间距**——查出真根因：`.mkt-card` 单独使用（不在 `.mkt-row` 的 grid 里）时本身不带 `margin-bottom`，阶段周期卡是因为额外有 `.mkt-phase{margin-bottom:20px}` 这条规则才有间隔，v799 加这张卡时漏了对应的 `.mkt-advice` 规则，导致卡片跟下方 VIX/VXN 那一排是零间距贴着的。补上 `.mkt-advice{margin-bottom:20px}`，与阶段周期同款。**④手机端排版**——`.mkp-advice-def` 桌面端是「圆点+名称+state+detail」一行三列固定宽度对齐，手机端改用 `grid-template-areas` 纵向堆叠（名称一行，state/detail 各自一行），横向挤在 390px 里三段会互相重叠。Playwright 回归（新增 `v800.js`，在 `v799.js` 基础上订正窗口断言并新增场景 10/11，共 62 项）：窗口确认不超过 200 个交易日、说明列表条数与图例状态数一致（只列出现过的）、内容与 `combineAxes` 的 `state`/`detail` 逐字核对、与下方模块间距 ≥15px（实测 20px）、手机端「为什么」文字换行不与名称同行挤压、手机端无横向溢出；`v782` 27 / `v793` 17 / `v795` 16 / `v796` 14 / `v798` 20 / `v787` 42 / `v792` 32 / `v794` 20 / `cycle5` 60 复跑全绿。 |
| v801 | **「建议走向」更名为「周期分析 · Cycle Analysis」+ 状态说明改为可收起 + 移到阶段周期正上方**（用户三条要求）。**①更名**——卡片标题从"建议走向 · Advice History"改为"周期分析 · Cycle Analysis"。**注意**：Market 页另有一张人工季度判据清单叫「周期系统分析 · CYCLE SYSTEM」（`renderCycleCard`，v760 起），跟这张自动回放的卡片名字很接近但是两个完全不同的模块——这是用户指定的命名，已在 CLAUDE.md 里特别标注，避免以后改错卡。**②状态说明改为可收起**——v800 新增的 `.mkp-advice-defs`（"是什么/为什么/怎么做"三段列表）此前是常驻展示，收进 `<details class="mkp-fold" data-mkp-fold="adv-defs">`，**默认收起**，折叠时只留「N 种状态」摘要行；跟卡片里原有的"建议切换" fold（`data-mkp-fold="adv"`）是两个**各自独立**的折叠小节，各自的 `localStorage` key（`trendo_mkp_adv-defs_open` / `trendo_mkp_adv_open`）互不联动，展开一个不会带动另一个。展开后的列表排版同步对齐——从裸列表改为套用"建议切换"的 `.mkp-trs` 同款带边框盒子（`border+radius+overflow:hidden`，逐行 `border-top` 分隔+隔行浅底色），两个折叠小节现在是同一套排版语言，不再是常驻列表和带框列表拼在一起的两套样式。**③位置**——`renderMarket` 里 `mkAdviceHTML(advice)` 从"综合建议横幅正下方"挪到紧邻在 `mkPhaseHTML(...)` **之前**，VIX/VXN 指标行退回综合建议横幅下方；两张回放色带卡（周期分析+阶段周期）现在挨在一起，方便对照着读同一段时间窗口。**排查记录**：确认新 fold 默认关闭态下量出的子元素坐标属实——这个环境的 Chromium 对未展开的 `<details>` 内容仍正常参与布局（`getBoundingClientRect()` 非零、`getComputedStyle().display` 报 `block`），跟 v762 记录的现象一致，判断"是否真的收起"必须量 `<details>` 元素本身、不能拿子元素坐标当证据；本版测试因此复用 `localStorage` 落盘状态与 `.open` 属性做真正的收起判定，不依赖子元素坐标。Playwright 回归（新增 `v801.js`，在 `v800.js` 基础上更新标题断言+新增场景 12/13，共 69 项）：卡片标题含「周期分析」、新 fold 默认收起且摘要行写明状态种数、展开后独立落盘且不联动「建议切换」fold、`mkt-advice` 与 `mkt-phase` 在 DOM 中紧邻相接（中间无其他模块）；`v782` 27 / `v793` 17 / `v795` 16 / `v796` 14 / `v798` 20 / `v787` 42 / `v792` 32 / `v794` 20 / `cycle5` 60 复跑全绿。`v800.js` 里断言旧标题「建议走向」与旧 `.mkp-filtered` 单选择器（现在页面里有两个同类节点，需要按所属的 details 区分）的 3 处断言已按新结构更新，其余逻辑不变、不作废——所以延续为 `v801.js`，不单独留一份旧套件。 |
...
**抽屉里跟价格有关的东西：**
`fetchPrices()` 不会整块重建抽屉（那会冲掉正在编辑的输入框、BX 页签、滚动位置），
只调 `syncDrawerLive()` 就地更新现价/涨幅/盈亏/R/level bar/盈利保护块。
**新增随价格变化的抽屉内容时，记得在 `syncDrawerLive()` 里补一处**，否则它会停在
抽屉打开那一刻——盈利保护的「已跌破」提示就是这么卡住过的。

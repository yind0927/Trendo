...
### 每批（周）的折叠卡头部——`N 周` / `N 周 已完成` / `不足 1 周`

批次列表（下方按周展开的 `<details class="mp-cohort">`）每一条的头部标签由 `mpHeadlineKey(c)`
决定该批**现在该用哪个持有期汇总展示**，三种状态：

```
已走到主口径(2周)及以上   → "2 周"           不带"已完成"（这是常态，主口径本身不需要额外说明）
只走到比主口径短的持有期  → "N 周 已完成"     N 是已完成的最长持有期，title 说明"尚未走到主口径，
                                              先按已完成的最长周期显示"
一个检查点都没到（刚录入）→ "不足 1 周"       v797 前这里错误地借用主口径的"2 周"当占位符，
                                              跟"真的走满 2 周"显示成同一句话，
                                              肉眼分不清——已修复
```

**这不是"第一批/第二批/第三批"的批次序号**，纯粹是"这一批到今天为止能报出的最长持有期"，
同一批会随着时间推移从"不足 1 周"→"1 周 已完成"→"2 周"（主口径）→（若还没到 2 周主口径，
更长周期到了也不会再变字面，因为一旦命中主口径就锁定显示主口径）逐步变化，跟它在列表里
排第几、是第几批完全无关——刚录入的最新一批显示"不足 1 周"、上上周的老批次显示"2 周"是
正常现象，不是乱序或 bug。

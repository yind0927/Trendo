...
| v510 | **手机端持仓表格布局修复**（真实仓+模拟仓列表视图）：v508 起表格改 `table-layout:fixed`+`width:100%`，桌面正常但手机端把全部 9 列压进 ~360px 视口导致文字重叠（如 `$63.$57.71.00`）。修法两步：①手机端（`max-width:768px`）列表视图隐藏 **止损/止盈** 两列——`visTableCols(isClosed)` 统一 `renderTable`/`renderSimTable` 的列过滤，`isMobileWidth()=matchMedia("(max-width:768px)")` 命中时从 `COLS` 里剔除 `stop`/`target`（数据仍可在抽屉编辑、level bar 上查看，仅列表列减负）；②手机端 `table.holdings,table.sim-holdings{min-width:560px}` 让表格宽于视口，触发外层 `overflow-x:auto` 横向滚动（首列 Ticker `position:sticky` 常驻），剩余 ~7 列获得可读宽度。`colgroupHTML` 按可见列自动重算百分比，隐藏两列后其余列自动摊分。init 里加 `matchMedia("(max-width:768px)").addEventListener("change", ...)` 监听断点跨越，横竖屏/窗口缩放时实时重渲染两张表以增删止损止盈列。桌面端保持全部列不变。 |
